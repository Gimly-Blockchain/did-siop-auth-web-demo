...
import {AuthResponse, StateMapping, QRVariables} from "@spostma/onto-demo-shared-types";
...
class Server {
        ...
        this.express.get("/ext/get-auth-request-url", (request, response) => {
            ...
            if (stateMapping) {
                ...
                this.rp.createAuthenticationRequest({
                    nonce,
                    state: stateId,
                    presentationDefinitions: this.buildPresentationDefinitions()
                }).then(requestURI => {
                    response.statusCode = 200
    ...
    private buildPresentationDefinitions() {
